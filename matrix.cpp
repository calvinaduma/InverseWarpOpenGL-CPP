...
void setbilinear(double width, double height, Vector2D xycorners[4], BilinearCoeffs &coeff){
  coeff.width = width;
  coeff.height = height;
  ...
  coeff.a3 = xycorners[2].x - xycorners[1].x - xycorners[3].x + xycorners[0].x;
  coeff.b3 = xycorners[2].y - xycorners[1].y - xycorners[3].y + xycorners[0].y;
...
void invbilinear(const BilinearCoeffs &c, Vector2D xy, Vector2D &uv){
  ...
  if(fabs(c.c2) <= EPSILON){
    uv.y = (c.a1 * xy.y - c.a1 * c.b0 - c.b1 * xy.x + c.a0 * c.b1) / (c.a1 * c.b2 - c.a2 * c.b1);
    uv.x = (xy.x - c.a0 - c.a2 * uv.y) / c.a1;
  } else {
    c0 = c.a1 * (c.b0 - xy.y) + c.b1 * (xy.x - c.a0);
    c1 = c.a3 * (c.b0 - xy.y) + c.b3 * (xy.x - c.a0) + c.a1 * c.b2 - c.a2 * c.b1;
    ...
    if(uv.y < 0.0 || uv.y > 1.0){
      uv.y =(-c1 - discriminant) / (2.0 * c.c2);
      uv.x = (xy.x - c.a0 - c.a2 * uv.y) / (c.a1 + c.a3 * uv.y);
    } else {
      uv.x = (xy.x - c.a0 - c.a2 * uv.y) / (c.a1 + c.a3 * uv.y);
